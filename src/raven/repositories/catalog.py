"""MongoDB catalog persistence; pages are separate records, never one oversized BSON blob."""
    ...
    def load_catalog(
        ...
        *,
        ...
        try:
            ...
            if payload is None:
                return None
            payload = {key: value for key, value in payload.items() if key != "_id"}
            pages = []
            if with_pages:
                for record in (
                    ...
                    for key in ("topics", "uses", "quotes", "dates", "places", "references"):
                        page[key] = tuple(page[key])
                    page["entities"] = tuple(tuple(item) for item in page["entities"])
